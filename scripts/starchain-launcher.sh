...
openclaw agent --agent gemini -m "【星链 Step 1.5A - 扫描】
...
openclaw agent --agent openai -m "【星链 Step 1.5B - 宪法】
...
openclaw agent --agent claude -m "【星链 Step 1.5C - 计划】
...
openclaw agent --agent gemini -m "【星链 Step 1.5D - 复核】
...
if [[ "$REVIEW_RESULT" == "MAJOR_DRIFT" ]] || [[ "$LEVEL" == "L3" ]]; then
  ...
  openclaw agent --agent openai -m "【星链 Step 1.5E - 仲裁】
...
openclaw agent --agent notebooklm -m "【星链 Step 1.5S-A - 深度研究】
...
openclaw agent --agent brainstorming -m "【星链 Step 1.5S-B - Spec-Kit】
...
openclaw agent --agent coding -m "【星链 Step 2 - 开发】
...
openclaw agent --agent claude -m "【星链 Step 3 - 主审查】
...
openclaw agent --agent gemini -m "【星链 Step 3 - Adversarial Review】
...
openclaw agent --agent test -m "【星链 Step 5 - 测试】
...
openclaw agent --agent docs -m "【星链 Step 6 - 文档】
